...
unsigned int nStakeMinAge = 24 * 60 * 60; // 4 hours
...
const string strMessageMagic = "Matrixcoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
   ...
   if (pindexBest->nHeight <= 1)
      {
        int64_t nSubsidy = 25000000 * COIN;
        return nSubsidy + nFees;
      }
      
    else if (pindexBest->nHeight <= 999999)
      {
        int64_t nSubsidy = 0.47 * COIN;
        return nSubsidy + nFees;
      }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Matrixcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
